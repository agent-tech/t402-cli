...
export async function runHelp(ctx: CliContext): Promise<number> {
  ...
  output(format, {
    ...
    payment_flow: [
      ...
      '2. Send: run `tpay send --to <recipient> --amount <n>` to create and execute a payment',
      '   - The CLI creates a payment intent with the T402 API',
      '   - Signs the transaction using the configured wallet (seed phrase)',
    ...
    stdin: 'send accepts JSON via stdin: {"to":"...","amount":"..."}',
